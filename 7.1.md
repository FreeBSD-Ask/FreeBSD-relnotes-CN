...
FreeBSD 7.1-RELEASE 的发行说明包含了对 FreeBSD 基本系统在 7.1-STABLE 开发线上的更改总结。本文档列出了自上一个版本以来发布的相关安全通告，并介绍了 FreeBSD 内核和用户空间的重大更改。还简要介绍了升级的相关事项。
...
典型的发行说明项目记录了 7.0-RELEASE 之后发布的最新安全通告、新的驱动程序或硬件支持、新的命令或选项、重要的 bug 修复或第三方软件升级。它们也可能列出对主要 Port/包或发布工程实践的更改。显然，发行说明无法列出从一个版本到另一个版本之间对 FreeBSD 所做的每一个更改；本文件主要集中在安全通告、用户可见的更改以及主要的架构改进上。

### 2.1 安全公告

以下安全通告中描述的问题已被修复。更多信息请查阅各个通告，详情请访问 [http://security.FreeBSD.org/](http://security.freebsd.org/)。
