...
FreeBSD 及 3.0-RELEASE 目录布局（特别是如果你是从软盘安装），请参阅 ABOUT.TXT。安装说明请参见 `INSTALL.TXT` 和 `HARDWARE.TXT` 文件。
...
- 现已实现异步 IO（目前在非 SMP 环境下），并增加了对内核辅助线程的支持。
...
- 内核中的 timeout(9) 系统已被彻底改进。这为调用插入和删除提供了 O(1) 的时间复杂度，而软时钟中处理的工作量为 O（哈希链长度）。可以在以下链接找到原始论文：<http://www.cs.berkeley.edu/~amc/research/timer/>
- 驱动程序缓冲队列的变化以处理有序事务。这旨在为文件系统代码中的数据和元数据写入排序提供支持，一旦完全支持后，将进一步实现。
- 现在支持 EISA 共享中断，工作框架最初用于支持 PCI 共享中断。
- 支持 Comtrol Rocketport 卡。
- IPFW 的数据包和字节计数器已从 32 位扩展到 64 位，已在 ipfw 中新增 `FWD` 操作以支持透明代理，且 divert 操作略有变化——更多信息请参阅 natd(8) 和 ipfw(8) 的手册页。
...
- 已将 **systat(1)**、**iostat(8)**、**rpc.rstatd(8)** 和 **vmstat(8)** 改进为使用新的 **devstat(3)** 库和 **devstat(9)** 统计子系统。这些工具（除了 **rpc.rstatd(8)**）现在打印出更有用的统计数据，并能够查看系统中所有设备的统计数据，而不仅限于前 8 个设备。
...
- WORM/CD-R/CD-RW 的写入支持由 **cdrecord** 提供，该工具可以在 Ports 树中找到
