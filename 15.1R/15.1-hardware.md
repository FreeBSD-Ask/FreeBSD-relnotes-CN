...
**mpt**
驱动所支持的 `Ultra 320 SCSI` 控制器芯片可在许多系统主板上找到，包括：
...
[sdhci(4)](https://man.freebsd.org/cgi/man.cgi?query=sdhci&sektion=4&format=html) 驱动支持 SD 主机控制器规范。 通过 PCI 总线连接时， 控制器会自动配置。 许多 SoC 芯片提供直接映射到 I/O 内存的 SDHCI 控制器。 对于这些情况，可使用
**fdt(4)**
或
**acpi(4)**
方法配置控制器，相关方法由主板厂商提供。
...
与大多数支持通用标准的驱动不同，
**sdhci**
需要大量 quirks 来应对硬件缺陷、 专有寄存器和定义不明确的电源管理。 虽然来自 Intel、Xilinx、Rockchip、Freescale、Ricoh 和 TI 的许多芯片组都有这些条目， 但使用某些控制器时可能产生次优性能。 当设备通过
**fdt(4)**
或
**acpi(4)**
...
**sym**
支持的 SCSI 控制器可内置于主板， 也可位于以下附加板卡上：
...
**ice**
驱动支持此系列中具有 SFP28/QSFP28 cages 的部分适配器， 这些适配器的固件要求使用 Intel 认证模块； 这些认证模块列出如下。 驱动无法禁用此认证检查。
...
**uhso**
驱动会自动将设备从“Zero-CD”模式切换到调制解调器模式。 可通过 **sysctl(8)** 将 `hw.usb.uhso.auto_switch` 设置为 0 来禁用此行为。
...
**bwi**
驱动使用 Broadcom 固件的旧版 v3。 虽然旧版固件支持大多数 BCM43xx 部件，但
**bwn(4)**
驱动对其支持的较新芯片工作得更好。 如果你使用较旧的 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2），必须使用
**bwi**
 驱动。
**bwn(4)**
使用的 v4 版本固件不支持这些芯片。
...
**bwi(4)**，
因为 v4 版本的固件不支持这些芯片。 较新的固件过大，无法装入这些旧芯片。
...
默认情况下，每个
**pcm(4)**
设备对应声卡上的一个物理端口。 对于 ADAT 端口，支持 8 通道、4 通道和 2 通道格式。 ADAT 通道的有效数量为：单速（32kHz-48kHz）8 通道，倍速（64kHz-96kHz）4 通道，四倍速（128kHz-192kHz）2 通道。 根据所选采样率和通道格式，并非所有 pcm 通道都能映射到 ADAT 通道，反之亦然。
