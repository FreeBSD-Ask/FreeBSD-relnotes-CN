...
典型的发行说明项目包括在 8.1-RELEASE 后发布的安全公告、新的驱动程序或硬件支持、新的命令或选项、重要的错误修复，或第三方软件升级。它们还可能列出重大端口/包或发布工程实践的变化。显然，发行说明不能列出 FreeBSD 在发布之间所做的每一个更改；本文档主要集中在安全公告、用户可见的变化以及主要的架构改进上。
