...
FreeBSD 及 3.3-RELEASE 目录结构（尤其是在从软盘安装时！），请参阅 `ABOUT.TXT`。安装说明请参阅文件 `INSTALL.TXT` 和 `HARDWARE.TXT`。
...
- `inetd(8)` 中的 TCP Wrapper 支持现在通过命令行选项进行控制，并且除了之前支持的服务类型之外，数据报（UDP）服务也可以被包装。详细信息请参阅手册页，因为没有命令行选项的 `inetd` 不会进行包装。
