...
此外，可将其写入 USB 闪存盘（U 盘）用于 amd64 架构，在可从 USB 启动的机器上进行安装。还支持启动到基于“livefs”的救援模式。不含预构建的第三方 package。
...
可写入 USB 闪存盘（U 盘）用于启动机器，但介质本身不包含安装发行版，与 bootonly 镜像类似。还支持启动到基于“livefs”的救援模式。不含预构建的第三方 package。
