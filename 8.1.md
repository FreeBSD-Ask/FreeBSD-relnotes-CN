...
所有用户在安装 FreeBSD 之前，建议先查阅发布勘误。勘误文档会随着发布周期结束或发布后发现的“最新”信息而更新。通常，它包含已知的 bug、安全公告和文档修正。FreeBSD 8.1-RELEASE 的最新勘误可以在 FreeBSD 网站上找到。

## 2 新特性

本节介绍了自 8.0-RELEASE 以来，FreeBSD 中最显著的用户可见的新特性或更改。

典型的发行说明条目包括 8.0-RELEASE 后发布的安全通告、新驱动程序或硬件支持、新命令或选项、主要的错误修复或第三方软件升级。它们还可能列出对主要的 Ports/包或发布工程实践的更改。显然，发行说明无法列出所有 FreeBSD 在两个版本之间的更改；本文档主要关注安全通告、用户可见的更改和主要的架构改进。

### 2.1 安全通告

以下安全通告中描述的问题已经得到修复。更多信息请参阅各个安全公告，地址为[http://security.FreeBSD.org/](http://security.freebsd.org/)。
...
新增了内核选项 `option ATA_CAM`。启用该选项后，将 [ata(4)] 控制器驱动转为 [cam(4)] 接口模块。启用此选项会弃用所有 [ata(4)] 外设驱动和接口（如 ad 和 acd），并允许原生使用 [cam(4)] 驱动 ada 和 cd 及其接口。请注意，默认情况下，在 GENERIC 内核中未启用此选项。
