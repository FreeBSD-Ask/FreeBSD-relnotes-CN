...
从具有互联网邮件连接的机器上提交错误报告的首选方法是使用 `send-pr` 命令，或使用以下链接的 CGI 脚本：`http://www.FreeBSD.org/send-pr.html`。 我们忠实的 bugfiler 程序会认真归档错误报告，你可以放心，我们会尽最大努力尽快响应所有报告的错误。通过这种方式提交的错误报告也将在我们的官网的支持部分显示，因此，它们不仅作为错误报告有价值，还为其他用户提供了潜在问题的“指示标记”。
