...
修复了 **sendmail** 中的一个远程可利用的缓冲区溢出漏洞。有关更多详细信息，请参见安全公告 [FreeBSD-SA-03:07](ftp://ftp.freebsd.org/pub/FreeBSD/CERT/advisories/FreeBSD-SA-03:07.sendmail.asc)。在 FreeBSD 4.8-RELEASE 中，此漏洞通过供应商提供的补丁进行了修复（但修复太晚，未能纳入发行说明）。在 FreeBSD 4.9-RELEASE 中，已通过引入新的 **sendmail** 版本进行修复。
