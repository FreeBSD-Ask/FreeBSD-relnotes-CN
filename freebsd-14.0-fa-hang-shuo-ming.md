...
新增了名为 `zpoolreguid` 的 RC 脚本，它将为一个或多个 zpool 分配新的 GUID，在共享数据集的虚拟化环境中很有用。
...
[bhyve(8)](https://man.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&format=html) 虚拟机管理程序和内核模块 [vmm(4)](https://man.freebsd.org/cgi/man.cgi?query=vmm&sektion=4&format=html) 现在在虚拟机中支持超过 16 个虚拟 CPU。默认情况下，Bhyve 允许每个虚拟机创建与主机上物理 CPU 数相同的虚拟 CPU。此限制可以通过 loader 可调参数 `hw.vmm.maxcpu` 进行调整。[ee98f99d7a68](https://cgit.freebsd.org/src/commit/?id=ee98f99d7a68)
