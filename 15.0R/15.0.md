...
工具 [ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 的选项 `-O` 现在更加灵活和可预测。[ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 显示的列列表现在会首先在不考虑选项 `-O` 的情况下构建。然后，所有通过 `-O` 传入的列将最终插入到已构建的显示列中——如果存在第一列 PID 列，则插在其后；否则插入到开头，并保持它们作为选项 `-O` 参数的出现顺序。[5dad61d9b949](https://cgit.freebsd.org/src/commit/?id=5dad61d9b949)（由 FreeBSD 基金会赞助）
