...
驱动支持以下来自 Adaptec AAC 家族的 Parallel SCSI、SATA 和 3G SAS RAID 控制器：
...
驱动通过 CAM 子系统支持经由 PCIe 或 NVMF（NVMe over Fabric）连接的 NVMe（Non-Volatile Memory Express）存储设备。
...
驱动支持 Promise Supertrak SX6000 ATA 硬件 RAID 控制器。

[sdhci(4)](https://man.freebsd.org/cgi/man.cgi?query=sdhci&sektion=4&format=html)

驱动支持 SD 主机控制器规范。通过 PCI 总线连接时，控制器会自动配置。许多 SoC 芯片提供直接映射到 I/O 内存的 SDHCI 控制器。对于这些情况，可使用 **fdt(4)** 或 **acpi(4)** 方法配置控制器，相关方法由主板厂商提供。
...
驱动为以下 Symbios/LSI Logic PCI SCSI 控制器提供支持：
...
驱动为基于 Attansic/Atheros L1 千兆以太网控制器芯片的 LOM 提供支持，包括：
...
驱动支持以下基于 ADMtek AN986 Pegasus 芯片组的 USB 快速以太网适配器：
...
驱动支持基于 ASIX Electronics AX88172/AX88178/AX88772/AX88772A/AX88772B/AX88760 的 USB 以太网适配器，包括：
...
驱动为基于 QLogic NetXtreme II 千兆以太网控制器家族的各类 NIC 提供支持，包括以下型号：
...
驱动为基于 Broadcom BCM570x 千兆以太网控制器芯片家族的各类 NIC 提供支持，包括以下型号：
...
驱动支持以下 Broadcom 10Gb 至 400Gb 以太网控制器：
...
驱动为基于 QLogic NetXtreme II 10Gb 以太网控制器芯片家族的各类 NIC 提供支持，包括以下型号：
