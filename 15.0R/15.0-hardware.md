...
[sis(4)](https://man.freebsd.org/cgi/man.cgi?query=sis&sektion=4&format=html) 驱动支持基于 Silicon Integrated Systems SiS 900 和 SiS 7016 的快速以太网适配器和嵌入式控制器，以及基于 National Semiconductor DP83815 (MacPhyter) 和 DP83816 芯片的快速以太网适配器。
支持的适配器包括：
...
[tsec(4)](https://man.freebsd.org/cgi/man.cgi?query=tsec&sektion=4&format=html) 驱动配合工作：
...
[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html) 驱动应当能与 Option 的大多数设备配合工作。
以下设备已验证可正常工作：
...
**uhso** 驱动会自动将设备从“Zero-CD”模式切换到调制解调器模式。
可通过 **sysctl(8)** 将
...
[iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 驱动支持来自
**mvm**
...
以下设备应能与
[uslcom(4)](https://man.freebsd.org/cgi/man.cgi?query=uslcom&sektion=4&format=html) 驱动配合使用：
...
[xhci(4)](https://man.freebsd.org/cgi/man.cgi?query=xhci&sektion=4&format=html) 驱动支持具有 PCI 类 12（串行总线）、子类 3（USB）和编程接口 48（XHCI）的
`XHCI`
兼容控制器。

### IEEE 1394（Firewire）设备

[fwohci(4)](https://man.freebsd.org/cgi/man.cgi?query=fwohci&sektion=4&format=html) 驱动为 PCI/CardBus FireWire 接口卡提供支持。
...
[hifn(4)](https://man.freebsd.org/cgi/man.cgi?query=hifn&sektion=4&format=html) 驱动支持包含 Hifn 7751、7951、7811、7955 和 7956
芯片组的各类卡，例如：
...
当前版本的
[netfpga10g_nf10bmac(4)](https://man.freebsd.org/cgi/man.cgi?query=netfpga10g_nf10bmac&sektion=4&format=html) 驱动适用于 NetFPGA-10G Embedded CPU Ethernet Core version 1.00a
的一个 PIO 模式接口。

以下 PLX/Avago/Broadcom 芯片由
[ntb_hw_plx(4)](https://man.freebsd.org/cgi/man.cgi?query=ntb_hw_plx&sektion=4&format=html)驱动支持：
