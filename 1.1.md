...
在 386BSD 0.1 的分发版本中，修复了数百个漏洞，并将许多过时的软件升级到 GAMMA 版本中的最新发布版。1.1 版本解决了 BETA 和 GAMMA 用户反馈的许多首次运行问题。
...
FreeBSD 的核心不包含 DES（数据加密标准）代码，这确保了它可以出口到美国以外的地区。核心发行版中为美国用户提供了一款附加包，其中包含通常使用 DES 的程序。分开提供的辅助软件包任何人都可以使用。
...
> **注意**: SCSI 控制器和 Mitsumi CDROM 接口卡（`mcd` 驱动）是当前连接 CD-ROM 驱动器的唯一方式。FreeBSD 不支持 SoundBlaster CDROM 接口和其他"mini SCSI"适配器。
...
需要指出的是，以免产生误解——"FreeBSD" 始终是一款免费软件。核心团队几乎没有人通过 FreeBSD 的发行版及相关活动获利。提供此信息仅作为公共服务，为希望通过网络以外的方式获取软件的用户提供帮助。
