...
新增的内核选项 `KVA_SPACE`，可以用来重新配置内核虚拟地址空间的大小。
...
FreeBSD 中的 TCP 实现现在实现了缓存，用于存储未完成的、接收到的 SYN 段。传入的 SYN 段现在会导致条目被放入缓存，直到 TCP 三次握手完成，此时会像往常一样为连接分配内存。此外，所有的 TCP 初始序列号（ISN）都作为 cookie 使用，从而允许在缓存中删除条目，但仍然可以接受其相应的 ACK。所谓的 `syncache` 和 `syncookies` 特性组合使得主机对基于 TCP 的拒绝服务攻击更加具有抵抗力。美国国防高级研究计划局和 NAI Labs 赞助了此功能的开发工作。
