...
所有用户在安装 FreeBSD 之前，建议先查阅发布的勘误文档。该文档会更新一些在发布周期末或发布后发现的“最新”信息。通常，它包含已知的 bug、安全公告和文档的修正。FreeBSD 11.4-RELEASE 的最新勘误副本可以在 FreeBSD 网站上找到。

本文档介绍了自 11.3-RELEASE 以来 FreeBSD 中最显著的新增或变更的功能。一般来说，本文描述的变更是 11.4-STABLE 分支特有的，除非特别标明为已合并（MERGED）的功能。

典型的发行说明项目会记录在 11.3-RELEASE 之后发布的安全公告、新的驱动程序或硬件支持、新的命令或选项、重要的 bug 修复或第三方软件升级。它们还可能列出对主要 Port/软件包或发布工程实践的变更。显然，发行说明无法列出每一个在发布之间对 FreeBSD 所做的变更；本文档主要聚焦于安全公告、用户可见的变更和主要的架构改进。
