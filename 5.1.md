...
已新增的 [vpd(4)](http://www.freebsd.org/cgi/man.cgi?query=vpd&sektion=4&manpath=FreeBSD+5.1-release) 驱动程序，用于从 IBM ThinkPad 机器的 Vital Product Data 结构读取硬件信息。

#### 2.2.2 启动加载器变更

启动加载器的两个部分（boot1 和 boot2）已合并为单独的启动文件，以简化需要写入或以其他方式操作启动加载器的程序。
