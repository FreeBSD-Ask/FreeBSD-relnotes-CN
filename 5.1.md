...
已新增对 Intel Pentium Pro 及更高处理器上的物理地址扩展（PAE）功能的支持。这允许机器使用最多 64GB 的 RAM，尽管任何单个进程（或 FreeBSD 内核）可用的内存量没有改变。更多信息请参见 [pae(4)](http://www.freebsd.org/cgi/man.cgi?query=pae&sektion=4&manpath=FreeBSD+5.1-release) 手册页面。美国国防高级研究计划局和 Network Associates 实验室赞助了此功能的开发。
...
1:1 线程包已实现（每个应用程序中的每个 pthread 都对应一个 KSE 和线程）。在此模型下，内核处理所有线程调度决策和所有信号传递。它使用一些通用的 KSE 代码，是 M:N 线程工作的受限版本。实现此功能的 libthr 库是 libc_r 库的替代品。请注意，libthr 目前并非默认构建。
