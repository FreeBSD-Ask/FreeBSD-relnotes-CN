...
已新增对 Intel Pentium Pro 及更高处理器上的物理地址扩展（PAE）功能的支持。这允许机器使用最多 64GB 的 RAM，尽管任何单个进程（或 FreeBSD 内核）可用的内存量没有改变。有关更多信息，请参阅 [pae(4)](http://www.freebsd.org/cgi/man.cgi?query=pae&sektion=4&manpath=FreeBSD+5.1-release) 手册页面。美国国防高级研究计划局和 Network Associates 实验室赞助了此功能的开发。
