...
本节介绍了自 5.0-RELEASE 以来，FreeBSD 中许多用户可见的新功能或变更。它包括 5-CURRENT 分支特有的项目，以及一些可能最近合并到其他分支（在 FreeBSD 5.0-RELEASE 之后）的功能。这些项目标记为[已合并]。
...
现已为内核线程实现部分懒惰切换机制；旨在减少不涉及其他进程的短上下文切换（如中断处理程序）的开销。可以通过选项 **LAZY_SWITCH** 启用此功能。
