...
新的 loader 可调变量 `net.fibs` 现在支持指定路由表的数量。`ROUTETABLES` 内核选项仍可用于设置路由表的默认数量。[[r235104](http://svn.freebsd.org/viewvc/base?view=revision&revision=235104)
...
[ata(4)](http://www.freebsd.org/cgi/man.cgi?query=ata&sektion=4&manpath=FreeBSD+8.4-RELEASE) 驱动程序现在在启用 `ATA_CAM` 内核选项时创建符号链接以保持向后兼容性。在启用 `ATA_CAM` 的内核中，ATA/SATA 磁盘被识别为 ada0 设备节点，而不是 ad0。自动生成符号链接 **/dev/ad0** 以指向 **/dev/ada0**，以保持向后兼容性。此符号链接生成可通过 `kern.cam.ada.legacy_aliases` 控制（在设置 `ATA_CAM` 时默认启用）。[[r234912](http://svn.freebsd.org/viewvc/base?view=revision&revision=234912)
...
** 1 ** 工具现在可以用 `PACKAGESUFFIX` 来指定包文件名中的扩展名。[[r240674](http://svn.freebsd.org/viewvc/base?view=revision&revision=240674)

** 1 ** 程序现在允许在 `-p` 选项中使用相对路径名。[[r233686](http://svn.freebsd.org/viewvc/base?view=revision&revision=233686)
...
[amd64, i386] 支持使用 ** 0 ** 工具进行版本间升级（以及各种安全分支的快照）。二进制升级过程将更新未修改的用户工具以及未修改的 **GENERIC** 内核，这些都是 FreeBSD 正式发布版本的一部分。** 1 ** 工具要求升级的主机具有互联网连接。

通过光盘分发媒体上的 ** 0 ** 主菜单的升级选项也支持一种较老形式的二进制升级。这种类型的二进制升级对于非 i386、非 amd64 的机器或没有互联网连接的系统可能很有用。
