...
强烈建议所有用户在安装 FreeBSD 前查阅发行说明中的勘误（更新说明）。该文档会在发布周期结束时或发布后更新，包含一些“临时”信息。通常，它会包括已知的漏洞、安全公告及文档的修正。FreeBSD 8.4-RELEASE 的最新勘误副本可以在 FreeBSD 网站上找到。
...
[ata(4)](http://www.freebsd.org/cgi/man.cgi?query=ata&sektion=4&manpath=FreeBSD+8.4-RELEASE) 驱动程序现在在启用 `ATA_CAM` 内核选项时创建符号链接以保持向后兼容性。在启用 `ATA_CAM` 的内核中，ATA/SATA 磁盘被识别为设备节点 ada0，而不是 ad0。自动生成符号链接 /dev/ad0 以指向 /dev/ada0，以保持向后兼容性。此符号链接生成可通过 `kern.cam.ada.legacy_aliases` 控制（在设置 `ATA_CAM` 时默认启用）。[[r234912](http://svn.freebsd.org/viewvc/base?view=revision&revision=234912)
