...
FreeBSD 7.2-RELEASE 的发行说明包含对 7.2-STABLE 开发线中 FreeBSD 基本系统所做更改的总结。本文档列出了自上次发布以来发布的相关安全通告，以及对 FreeBSD 内核和用户空间的重大更改。还提供了一些关于升级的简要说明。
...
建议所有用户在安装 FreeBSD 之前查阅发布错误文档。该文档会根据发布周期结束时或发布后发现的最新信息更新。通常，它包含已知的错误、安全通告和文档修正。FreeBSD 7.2-RELEASE 的最新错误文档可以在 FreeBSD 网站上找到。
...
以下安全通告中介绍的问题已被修复。更多信息请参考来自 [http://security.FreeBSD.org/](http://security.freebsd.org/) 的各个安全通告。
