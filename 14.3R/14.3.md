...
为 [mountd(8)](https://man.freebsd.org/cgi/man.cgi?query=mountd&sektion=8&format=html) 定义了新的命令行选项 `-a`。当文件系统使用 `-alldirs` 标志导出时，即使路径并非服务器的文件系统挂载点，导出操作也会成功。 [ead3cd3ef628](https://cgit.freebsd.org/src/commit/?id=ead3cd3ef628)
