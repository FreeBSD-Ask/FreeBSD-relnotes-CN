...
除了基本发行版，FreeBSD 还提供了一个全新的 Ports 软件集合，其中包含约 270 个常见程序。从 HTTP（WWW）服务器到游戏、编程语言、编辑器，几乎无所不包。整个 Ports 集合仅需 10MB 存储空间，每个 Port 都以与其原始源码的“增量差异”表示。这使得我们可以更容易地更新 Ports，并大大减少旧 1.0 版本 Ports 集合的磁盘空间需求。
