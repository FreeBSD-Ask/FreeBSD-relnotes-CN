...
我们还认真倾听了众多用户的意见和建议，努力提供一个更加合理、易于理解的安装流程。我们非常欢迎你对这一持续改进的过程提供反馈！

除了基本发行版，FreeBSD 还提供了一个全新的 Ports 软件集合，其中包含约 270 个广受欢迎的程序。从 HTTP（WWW）服务器到游戏、编程语言、编辑器，几乎无所不包。整个 Ports 集合仅需 10MB 存储空间，每个 Port 都以与其原始源码的“增量差异”表示。这使得我们可以更容易地更新 Ports，并大大减少旧 1.0 版本 Ports 集合的磁盘空间需求。
...
请参阅以下文件以了解 Ports 集合的详细说明：**/usr/share/FAQ/Text/ports.FAQ**
...
如果你只需要 FreeBSD 的密码安全性，而无需从其他主机（如 Sun 或 DEC 机器）复制加密密码到 FreeBSD 密码条目中，那么 FreeBSD 的基于 MD5 的安全性可能完全能够满足你的需求。我们相信，默认的安全模型在功能上完全可以与 DES 媲美，而且没有任何复杂的出口问题。如果你位于美国之外（甚至在美国内部），不妨试试！
...
在 2.0.5 之前，FreeBSD 会在探测 SCSI 设备时动态分配设备编号，导致 SCSI 设备故障可能改变设备编号，使仍然正常工作的磁盘上的文件系统无法挂载。硬编码则可基于 SCSI ID 和总线的静态分配设备编号（即设备名称）。SCSI 配置在内核配置文件中进行。可以在 `scsi(4)` 手册页或 `LINT` 内核配置文件中找到配置语法示例。
...
FreeBSD 现在支持一种“slice”抽象，使其与其他操作系统分区的互操作性更强。这项支持将允许 FreeBSD 使用 DOS 扩展分区。
...
现已增加对使用 Ontrack Disk Manager 的磁盘的支持。然而，`fdisk` 程序并不识别它，因此所有变更请使用 `boot.flp` 中的安装程序或在 DOS 下使用 Ontrack Disk Manager 工具。
