...
驱动程序 [twa(4)](http://www.freebsd.org/cgi/man.cgi?query=twa&sektion=4&manpath=FreeBSD+6.0-RELEASE) 已更新为从 3ware 网站分发的 9.2 版本（适用于 FreeBSD 5.2.1）。
...
在之前的 FreeBSD 版本中，disc1 CD-ROM（或 ISO 镜像）是可启动的安装盘，包含基本系统、Ports 和常用软件包。disc2 CD-ROM（或 ISO 镜像）是可启动的“修复盘”，带有实时文件系统，用于紧急修复。此布局现已变更。
