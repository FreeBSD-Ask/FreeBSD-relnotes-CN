...
FreeBSD 及 3.1-RELEASE 目录布局（特别是如果你是从软盘安装），请参阅 `ABOUT.TXT` 文件。有关安装说明，请参阅文件 `INSTALL.TXT` 和 `HARDWARE.TXT`。
...
- Steve Price 在构建和组织软件包方面做了大量工作，Satoshi Asami 一如既往地负责 Ports 的工作。
