...
包含安装 FreeBSD 基本系统、文档、调试发行版和少量预构建 package（旨在启动图形工作站）所需的全部内容。还支持启动到基于“livefs”的救援模式。若你能刻录并使用 DVD 介质，这应是你所需的全部。
...
包含 FreeBSD 基本系统。还支持启动到基于“livefs”的救援模式。不含预构建的 package。
