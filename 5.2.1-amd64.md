...
[**mount_msdosfs(8)**](http://www.freebsd.org/cgi/man.cgi?query=mount_msdosfs&sektion=8&manpath=FreeBSD+5.2.1-RELEASE) 工具现在支持 `-D` 选项（用于指定 MS-DOS 代码页）和 `-L` 选项（用于指定本地字符集）。这些选项用于转换文件名的字符集。**/usr/libdata/msdosfs** 表不再使用。
