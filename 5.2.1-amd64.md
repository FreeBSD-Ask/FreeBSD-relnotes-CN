...
许多 **/bin** 和 **/sbin** 中的工具现在作为静态链接的“crunched”二进制文件提供，存放在 **/rescue** 中。该功能类似于 [**sysinstall(8)**](http://www.freebsd.org/cgi/man.cgi?query=sysinstall&sektion=8&manpath=FreeBSD+5.2.1-RELEASE) 安装的 **/stand** 目录，但 **/rescue** 包含更多功能，并且作为 `buildworld`/`installworld` 操作的一部分更新。更多细节请参见 [**rescue(8)**](http://www.freebsd.org/cgi/man.cgi?query=rescue&sektion=8&manpath=FreeBSD+5.2.1-RELEASE)。
...
*强烈* 现有 FreeBSD 系统的用户建议阅读《FreeBSD 5.2.1-RELEASE 早期采用者指南》。此文档通常以 `EARLY.TXT` 为文件名，位于发布媒体或其他可以找到发行说明的地方。它提供了有关升级的一些说明，更重要的是，还讨论了升级到 FreeBSD 5.X 与继续使用 FreeBSD 4.X 的相对优点。
