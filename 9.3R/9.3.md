...
[find(1)](https://man.freebsd.org/cgi/man.cgi?query=find&sektion=1&format=html) 工具已更新以实现 `-ignore_readdir_race`。在此更改之前，`-ignore_readdir_race` 是 GNU [find(1)](https://man.freebsd.org/cgi/man.cgi?query=find&sektion=1&format=html) 兼容选项，如果指定则会被忽略。现在也有了一个计数主选项 `-noignore_re