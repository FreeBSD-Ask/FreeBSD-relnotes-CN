...
驱动 [ath(4)](http://www.freebsd.org/cgi/man.cgi?query=ath&sektion=4&manpath=FreeBSD+6.0-RELEASE) 已更新，将传输速率控制算法拆分为单独的模块。在使用 [ath(4)](http://www.freebsd.org/cgi/man.cgi?query=ath&sektion=4&manpath=FreeBSD+6.0-RELEASE) 驱动时，必须在内核配置中包含 `device ath_rate_onoe`、`device ath_rate_amrr` 或 `device ath_rate_sample` 之一。
...
已移除驱动程序 [wd(4)](http://www.freebsd.org/cgi/man.cgi?query=wd&sektion=4&manpath=FreeBSD+6.0-RELEASE)。由于 [ata(4)](http://www.freebsd.org/cgi/man.cgi?query=ata&sektion=4&manpath=FreeBSD+6.0-RELEASE) 驱动程序在 pc98 平台上运行良好，不再需要旧的 [wd(4)](http://www.freebsd.org/cgi/man.cgi?query=wd&sektion=4&manpath=FreeBSD+6.0-RELEASE) 驱动程序。
