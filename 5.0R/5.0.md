...
[fsck_ffs(8)](http://www.freebsd.org/cgi/man.cgi?query=fsck_ffs&sektion=8&manpath=FreeBSD+5.0-RELEASE) 现在支持通过选项 `-B` 对已挂载的 FFS 文件系统进行后台文件系统检查（这些文件系统必须启用软更新）。`-F` 参数现在决定是否需要对指定的文件系统进行前台检查。
