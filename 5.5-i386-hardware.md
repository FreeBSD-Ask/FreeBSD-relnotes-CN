...
对于所有受支持的 SCSI 控制器，全面支持 SCSI-I、SCSI-II 和 SCSI-III 外设，包括硬盘、光盘、磁带机（包括 DAT、8mm Exabyte、Mammoth 和 DLT）、介质更换器、处理器目标设备和 CD-ROM 驱动器。支持 CD-ROM 命令的 WORM 设备由 CD-ROM 驱动程序（如 `cd(4)`）支持只读访问。WORM/CD-R/CD-RW 写入支持由 `cdrecord(1)` 提供，它是 Ports中 `sysutils/cdrtools` port 的一部分。
