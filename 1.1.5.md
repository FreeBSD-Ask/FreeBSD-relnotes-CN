...
自从我们发布 FreeBSD 1.0 版本以来，已经修复了数百个错误，新增了功能，系统整体质量有了几乎不可估量的提升。FreeBSD 1.1.5 是近 18 个月的工作成果，我们的全志愿者工作组投入了数千工时。我们希望你喜欢这个版本！
...
通过 IDE 控制器（不使用 DMA）、真正的 EISA 控制器（如 UltraStor 和 Adaptec 1742A）和大多数 VLB（局部总线）控制器，可以完全避免这个问题。在必要的情况下，系统将使用"回弹缓冲区"（bounce buffers）与控制器进行通信，这样即使内存超过 16MB，也能顺利运行。
...
需要指出的是，以免你产生误解——“FreeBSD” 始终是免费软件。在“核心团队”中几乎没有人从发行版和与 FreeBSD 相关的任何事务中获利。我们仅提供这些信息，作为公共服务，帮助那些希望从网络以外的地方获取发布版的用户（而且获取我们的软件越方便，我们越高兴）。
...
另外，由于这是一项志愿者项目，我们始终欢迎愿意提供帮助的额外力量——目前还有远远更多的增强功能需要完成，我们自己无法独立完成！如有技术问题或愿意提供帮助，可以通过邮件与我们联系：`FreeBSD-hackers@freefall.cdrom.com`
...
FreeBSD 代表了来自世界各地数十位，甚至数百位个人的集体努力，他们辛勤工作为你带来了这一版本。列举所有为 FreeBSD 做出贡献的人是非常困难的，甚至可以说是不可能的，但我们仍然会尽力列出（按字母顺序排列，当然）。如果你的名字没有被提及，请放心，这完全是疏漏所致。
