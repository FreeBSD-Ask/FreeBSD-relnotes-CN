...
新增了 [rc.conf(5)](https://man.freebsd.org/cgi/man.cgi?query=rc.conf&sektion=5&format=html) 变量 `linux_mounts_enable`，用于控制在设置 `linux_enable` 为 `YES` 时是否在 **/compat/linux** 中挂载 Linux(R) 特定的文件系统。[[r364883]](http://svn.freebsd.org/viewvc/base?view=revision&revision=364883)（由 FreeBSD 基金会赞助）
...
[ena(4)](https://man.freebsd.org/cgi/man.cgi?query=ena&sektion=4&format=html) 驱动程序已更新至版本 2.2.0。[[r365381]](http://svn.freebsd.org/viewvc/base?view=revision&revision=365381)（由亚马逊公司赞助）
