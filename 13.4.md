...
建议用户在安装 FreeBSD 之前，先查阅发行勘误（errata）。勘误文件包含在发布周期后期及发布后发现的最新信息，通常包括已知错误、安全公告和文档修正。可在 FreeBSD 官方网站上找到 FreeBSD 13.4-RELEASE 的最新勘误。
...
- 对原生及基于 LinuxKPI 的无线驱动程序进行了许多稳定性修复。（由 FreeBSD 基金会赞助）
...
FreeBSD stable/14 及更早的分支仍将保留现有的 32 位内核和用户空间支持。在 stable/14 及更早的分支中，Ports 仍将支持构建 32 位系统的 ports 和软件包，前提是这些分支仍受 Ports 支持。然而，所有 32 位平台均为二级架构或三级架构，随着上游软件逐步弃用 32 位平台，部分 ports 的支持可能会逐渐下降。
