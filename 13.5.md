...
[shar(1)](https://man.freebsd.org/cgi/man.cgi?query=shar&sektion=1&format=html) 已被废弃，并已添加废弃通知。[0d946859c994](https://cgit.freebsd.org/src/commit/?id=0d946859c994)。`shar(1)` 程序虽然简单，但 sh 存档的基本思想充满风险，最好不要像 `$PATH` 中的程序和 man 页面那样被显著推广。可以通过 [tar(1)](https://man.freebsd.org/cgi/man.cgi?query=tar&sektion=1&format=html) 实现相同的功能。
