...
* FreeBSD "base" 系统现在可以使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 包管理器进行安装和管理（参见下文 pkgbase）。
...
FreeBSD 15.0 的一项重大变更是引入了使用 [pkg(8)](https://man.freebsd.org/cgi/man.cgi?query=pkg&sektion=8&format=html) 包管理器安装和管理基本系统的新方法。
...
包含 FreeBSD 基础操作系统。还支持启动到基于“livefs”的救援模式。不含预构建的第三方 package。
...
此外，可将其写入 USB 闪存盘（U 盘）用于 amd64 架构，在可从 USB 启动的机器上进行安装。还支持启动到基于“livefs”的救援模式。不含预构建 package。
...
可写入 USB 闪存盘（U 盘），用于在可从 USB 启动的机器上进行安装。还支持启动到基于“livefs”的救援模式。不含预构建的第三方 package。
...
可写入 USB 闪存盘（U 盘）用于启动机器，但介质本身不包含安装发行版，与 bootonly 镜像类似。还支持启动到基于“livefs”的救援模式。不含预构建 package。
