...
FreeBSD 15.0-RELEASE 的发行说明是对在 15-STABLE 开发线上对 FreeBSD 基本系统所做的变动总结。本文档列出了适用于自上个 RELEASE 发布以来的安全公告，以及对 FreeBSD 内核和用户空间的重要变动。同时也简要给出了一些关于更新的说明。

## 引言

本文件是 FreeBSD 15.0-RELEASE 的发行说明。它涉及了 FreeBSD 最近新增、变动及删除的特性，并提供了旧版本 FreeBSD 更新时的若干说明。

这些发行说明所对应的“RELEASE”发行版，代表 15-STABLE 开发分支上介于 14.0-RELEASE 与后续 15.1-RELEASE 之间的最新节点。可在 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 找到关于此分支上预构建二进制“RELEASE”发行版的信息。要获取此（或其他）FreeBSD“RELEASE”发行版的更多信息，可参阅 [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/) 的附录[获取 FreeBSD](https://docs.freebsd.org/en/books/handbook/mirrors)。
...
典型的发行说明项目有：在 14.0-RELEASE 之后发布的近期安全公告、新的驱动/硬件支持、新的命令/选项、主要缺陷修复/第三方软件更新。它们也可能列出对主要 Ports / 软件包，发行工程实践的变动。当然，发行说明不可能列出 FreeBSD 在两个发行版本之间的每一项变动；本文主要聚焦安全公告、用户可见变动以及主要架构改进。
...
现在工具 [mdo(1)](https://man.freebsd.org/cgi/man.cgi?query=mdo&sektion=1&format=html) 能在目标凭据中完全指定所有用户和组。作为便捷功能，除了完全显式指定外，还能以基线开始，为所有属性提供默认值（可以是密码数据库中某个用户的登录凭据或当前凭据），然后选择性修改这些属性。更新了手册页，描述了新选项及其相互作用。[4ffcb1a4a99c](https://cgit.freebsd.org/src/commit/?id=4ffcb1a4a99c) **【由 FreeBSD 基金会和谷歌（2025 谷歌编程之夏）赞助】**
...
Kerberos v5 数据库管理程序现在可以以一种格式导出 Heimdal KDC 数据库，使其可被 MIT KDC 导入。如何使用 `kadmin -l dump -f` 转移/转换 KDC 数据库，请参阅 [https://wiki.freebsd.org/Kerberos/Heimdal2MIT_KDC_Migration](https://wiki.freebsd.org/Kerberos/Heimdal2MIT_KDC_Migration)。[9fd3b28d4e0d](https://cgit.freebsd.org/src/commit/?id=9fd3b28d4e0d), [23fbea8cf2f3](https://cgit.freebsd.org/src/commit/?id=23fbea8cf2f3)
