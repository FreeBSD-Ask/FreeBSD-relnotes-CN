...
这些发行说明所对应的“RELEASE”发行版，代表 15-STABLE 开发分支上介于 14.0-RELEASE 与后续 15.1-RELEASE 之间的最新节点。关于此分支上预构建二进制“RELEASE”发行版的信息，可在 [https://www.FreeBSD.org/releases/](https://www.freebsd.org/releases/) 找到。要获取此（或其他）FreeBSD“RELEASE”发行版的更多信息，可参阅 [FreeBSD 手册](https://docs.freebsd.org/en/books/handbook/) 的附录[获取 FreeBSD](https://docs.freebsd.org/en/books/handbook/mirrors)。

我们强烈建议所有用户在安装 FreeBSD 之前查阅发行勘误。发行勘误文档会随着发行周期末期或发行之后发现的“最新”信息而更新。通常它包含了已知缺陷、安全公告以及文档修正。FreeBSD 15.0-RELEASE 的最新发行勘误可在 FreeBSD 网站上找到。

本文档包含自 14.0-RELEASE 以来，在 FreeBSD 中对用户最为可见的新特性或变动。通常除非特别标记为特性“[已合并]”，此文档所述变动均为 15-STABLE 分支所独有。

典型的发行说明项目会记录：在 14.0-RELEASE 之后发布的近期安全公告、新的驱动/硬件支持、新的命令/选项、主要缺陷修复/第三方软件更新。它们也可能列出主要 Ports / 软件包以及发行工程实践的变动。当然，发行说明不可能列出 FreeBSD 在两个发行版本之间的每一项变动；本文主要聚焦安全公告、用户可见变动以及主要架构改进。
...
现在工具 [mdo(1)](https://man.freebsd.org/cgi/man.cgi?query=mdo&sektion=1&format=html) 能在目标凭据中完全指定所有用户和组。作为便捷功能，除了完全显式指定外，还能以基线开始，为所有属性提供默认值（可以是密码数据库中某个用户的登录凭据或当前凭据），然后选择性修改这些属性。手册页已更新，描述了新选项及其相互作用。[4ffcb1a4a99c](https://cgit.freebsd.org/src/commit/?id=4ffcb1a4a99c) **【由 FreeBSD 基金会和谷歌（2025 谷歌编程之夏）赞助】**
...
构建系统 [bmake(1)](https://man.freebsd.org/cgi/man.cgi?query=bmake&sektion=1&format=html) 已更新至 2025-08-04，提供了多项调试改进，修复了如检测并拒绝 `gmake` 语法的 bug，并带来了功能改进，例如 `-j` 接受浮点数参数时将其作为可用 CPU 数量的倍数。
...
驱动 [iwlwifi(4)](https://man.freebsd.org/cgi/man.cgi?query=iwlwifi&sektion=4&format=html) 已更新至 Linux v6.17。基于 BE200 的芯片组需要更新固件，并需要额外的驱动修复，而这些修复未包含在本版本中。[69caa1cf3ce5](https://cgit.freebsd.org/src/commit/?id=69caa1cf3ce5) **（由 FreeBSD 基金会赞助）**
...
系统已添加 [syscons(4)](https://man.freebsd.org/cgi/man.cgi?query=syscons&sektion=4&format=html) 的弃用通知。[syscons(4)](https://man.freebsd.org/cgi/man.cgi?query=syscons&sektion=4&format=html) 与 UEFI 不兼容，不支持 UTF-8，并且使用 Giant 锁。目前尚无具体移除时间表，但对 Giant 锁的支持预计将在一到两个主要版本周期内取消。[8c922db4f3d9](https://cgit.freebsd.org/src/commit/?id=8c922db4f3d9) **（由 FreeBSD 基金会赞助）**

系统已移除工具 `shar`，但用户仍可通过 Ports [sysutils/freebsd-shar](https://cgit.freebsd.org/ports/tree/sysutils/freebsd-shar/) 使用。[3fde39073c72](https://cgit.freebsd.org/src/commit/?id=3fde39073c72)

由于安全性不足，OpenSSH 遵循上游更新，已移除 DSA 签名算法。

系统已移除数据库 [publickey(5)](https://man.freebsd.org/cgi/man.cgi?query=publickey&sektion=5&format=html)，该数据库使用 DES 加密，我们希望 2025 年没人会再使用它。[9197c04a251b](https://cgit.freebsd.org/src/commit/?id=9197c04a251b)
...
系统已废弃 [sound(4)](https://man.freebsd.org/cgi/man.cgi?query=sound&sektion=4&format=html) 中的 `unit.*` 代码，作为其中一部分，还移除了 [loader(8)](https://man.freebsd.org/cgi/man.cgi?query=loader&sektion=8&format=html) 可调项 `hw.snd.maxunit`。[25723d66369f](https://cgit.freebsd.org/src/commit/?id=25723d66369f) **（由 FreeBSD 基金会赞助）**
...
默认情况下，无特权进程不再能够调试、调度或向子 Jail 中属于同一 UID 的进程发送信号。系统已添加 [jail(8)](https://man.freebsd.org/cgi/man.cgi?query=jail&sektion=8&format=html) 设置 `allow.unprivileged_parent_tampering`，用于在父 Jail 和子 Jail 中的 UID 预期为同一用户时恢复先前行为。[8a5ceebece03](https://cgit.freebsd.org/src/commit/?id=8a5ceebece03)
...
对 vinum 卷的支持已移除。[f87bb5967670](https://cgit.freebsd.org/src/commit/?id=f87bb5967670), [e51036fbf3f8](https://cgit.freebsd.org/src/commit/?id=e51036fbf3f8)
...
对 802 标准网络的 ARP（[arp(4)](https://man.freebsd.org/cgi/man.cgi?query=arp&sektion=4&format=html)）支持已恢复；此前在移除 FDDI 支持时该支持被意外删除（这与以太网标准封装不同）。[d776dd5fbd48](https://cgit.freebsd.org/src/commit/?id=d776dd5fbd48)
...
协议 [pfsync(4)](https://man.freebsd.org/cgi/man.cgi?query=pfsync&sektion=4&format=html) 已更新，可同步多个缺失属性。这修复了带有选项 route-to、af-to、rtable、dummynet、tags 和 scrub 的状态同步问题。如果需要与旧版本 FreeBSD 进行同步，可通过 `ifconfig pfsync0 version $VERSION` 配置协议版本。对于 `$VERSION`：13.X 版本使用 1301，14.X 版本使用 1400。对于运行 FreeBSD 15.0 的主机之间的同步，默认值为 `1500`。[99475087d63b](https://cgit.freebsd.org/src/commit/?id=99475087d63b) **（由 InnoGames GmbH 赞助）**
...
支持 FreeBSD 7 和 8 的 [ipfw(8)](https://man.freebsd.org/cgi/man.cgi?query=ipfw&sektion=8&format=html) 二进制兼容代码已移除。[660255be1ed9](https://cgit.freebsd.org/src/commit/?id=660255be1ed9) **（由 FreeBSD 基金会赞助）**
...
在使用启动 ROM（即不使用 bhyveload(8)）运行 [bhyve(8)](https://man.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&format=html) 虚拟机时，bhyve 现在假设启动 ROM 会启用 PCI BAR 解码。这与某些启动 ROM（尤其是过时的 `edk2-bhyve` 构建）不兼容。要恢复旧行为，请在 bhyve 配置中添加 `pci.enable_bars='true'`。注意，软件包 `uefi-edk2-bhyve` 已重命名为 `edk2-bhyve`。[e962b37bf0ff](https://cgit.freebsd.org/src/commit/?id=e962b37bf0ff) **（由创新英国赞助）**

amd64 [bhyve(8)](https://man.freebsd.org/cgi/man.cgi?query=bhyve&sektion=8&format=html) 的选项 `lpc.bootrom` 和 `lpc.bootvars` 已被弃用，请改用顶层选项 `bootrom` 和 `bootvars`。[43caa2e805c2](https://cgit.freebsd.org/src/commit/?id=43caa2e805c2) **（由创新英国赞助）**
...
`virtual_oss` 已导入到基本系统。从 FreeBSD 15.0 开始，将不再构建 Port `audio/virtual_oss`。对于用户而言，唯一实际变化是安装过程：基本系统已提供所有功能，以下可选组件仍可通过 Ports 安装：
...
手册 [build(7)](https://man.freebsd.org/cgi/man.cgi?query=build&sektion=7&format=html) 已更新，加入了从源代码构建系统的说明。[275f61111f43](https://cgit.freebsd.org/src/commit/?id=275f61111f435)
...
[ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 手册已重写，解释了基本原理，并更新了其中的说明来匹配实际情况。前言部分已改写，对 [ps(1)](https://man.freebsd.org/cgi/man.cgi?query=ps&sektion=1&format=html) 命令的各个方面提供了全面概述。若干选项和关键词的说明已修正来匹配其实际行为。还增补了 STANDARDS 和 BUGS 部分。[ddf144a04b53](https://cgit.freebsd.org/src/commit/?id=ddf144a04b53) **（由 FreeBSD 基金会赞助）**

[mac_do(4)](https://man.freebsd.org/cgi/man.cgi?query=mac_do&sektion=4&format=html) 手册页已重写，以支持将多个用户和组作为单条规则的目标，这导致规则语法发生了变化。特别地，新增了 JAIL SUPPORT 和 SECURITY CONSIDERATIONS 部分。[bc201841d139](https://cgit.freebsd.org/src/commit/?id=bc201841d139) **（由 FreeBSD 基金会赞助）**
