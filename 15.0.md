...
Jail 和 Jail 描述符现在有关联的过滤器 [kevent(2)](https://man.freebsd.org/cgi/man.cgi?query=kevent&sektion=2&format=html)，可跟踪 Jail 的创建、变动、附加和移除。[1bd74d201a53](https://cgit.freebsd.org/src/commit/?id=1bd74d201a53) [9d7f89ef2607](https://cgit.freebsd.org/src/commit/?id=9d7f89ef2607)
...
默认情况下，无特权进程不再能够调试、调度或向子 Jail 中属于同一 UID 的进程发送信号。已添加 [jail(8)](https://man.freebsd.org/cgi/man.cgi?query=jail&sektion=8&format=html) 设置 `allow.unprivileged_parent_tampering`，用于在父 Jail 和子 Jail 中的 UID 预期为同一用户时恢复先前行为。[8a5ceebece03](https://cgit.freebsd.org/src/commit/?id=8a5ceebece03)
