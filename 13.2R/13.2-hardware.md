...
大多数带 SFP+/SFP28/QSFP+ 插槽的 Intel Ethernet 700 系列适配器的固件要求使用 Intel 认证模块；这些认证模块如下所列。
驱动无法禁用此认证检查。
...
[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html) 驱动使用以下适配器进行了测试：
