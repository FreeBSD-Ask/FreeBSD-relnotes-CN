...
FreeBSD 6.3-RELEASE 谨献给 Dr. Jun-ichiro Hagino，互联网社区中以其名 itojun 广为人知，以纪念其在 IPv6 协议上的远见卓识工作以及对互联网和 BSD 社区的众多其他贡献。
...
大多数架构的发行版 ISO 镜像内容已发生变化。以 i386 架构为例，存在名为“bootonly”、“disc1”、“disc2”、“disc3”和“docs”的 ISO 镜像。“bootonly”镜像适用于引导机器进行基于 FTP 或 NFS 的网络安装。“disc1”、“disc2”和“disc3”镜像是 CDROM 尺寸，用于进行包含基本软件包集合的完整安装，安装过程中无需访问 FTP 或 NFS 服务器。此外，“disc1”支持引导进入“基于 live CD 的文件系统”和系统救援模式。“docs”镜像包含所有支持语言的所有文档。大多数用户会发现“disc1”、“disc2”和“disc3”即可满足需求。如果打算从源码安装 Ports 而非使用发行版附带的预编译软件包，则仅需“disc1”。

也可从多家供应商处购买 CD-ROM 形式的 FreeBSD 6.3-RELEASE。将提供 FreeBSD 6.3 相关产品的供应商之一是：
...
从 FreeBSD 6.3 开始，[freebsd-update(8)](https://man.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&manpath=FreeBSD+6.3-RELEASE) 工具支持对运行早期 FreeBSD 发行版、发行候选和测试版的 i386 和 amd64 系统进行二进制升级。从较旧发行版（特别是早于 6.3-RC1 的版本）升级到 FreeBSD 6.3 的用户需要下载支持升级到新发行版的 freebsd-update(8) 更新版本。
...
许多公司捐赠了设备、网络访问或人力时间，以资助 FreeBSD 6.3 的发布工程活动，包括 The FreeBSD Foundation、FreeBSD Systems、Hewlett-Packard、Yahoo!、Network Appliances 和 Sentex Communications。
