...
[random(4)](http://www.freebsd.org/cgi/man.cgi?query=random&sektion=4&manpath=FreeBSD+5.0-RELEASE) 设备已重写为使用 **Yarrow** 算法。它从多种中断源收集熵，包括控制台设备、以太网和点对点网络接口以及大容量存储设备。现在，设备 [random(4)](http://www.freebsd.org/cgi/man.cgi?query=random&sektion=4&manpath=FreeBSD+5.0-RELEASE) 的熵会定期保存到 **/var/db/entropy** 文件中，也会在关机时保存。**/dev/random** 的语义发生了变化；它不再等待熵位，而是生成伪随机数据流，并且行为与 **/dev/urandom** 完全一致。
