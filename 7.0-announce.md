...
- jemalloc，一个全新的、高度可扩展的用户级内存分配器。
- [freebsd-update(8)](http://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&manpath=FreeBSD+7.0-RELEASE) 除了安全修复和勘误补丁外，还提供对官方支持的到新发行版的二进制升级。
...
发行版中所提供 ISO 镜像的内容对大多数架构已发生变更。以 i386 架构为例，存在名为 `bootonly`、`disc1`、`disc2`、`disc3`、`livefs` 和 `docs` 的 ISO 镜像。`bootonly` 镜像适用于引导机器通过 FTP 或 NFS 进行基于网络的安装。`disc1`、`disc2` 和 `disc3` 镜像用于进行包含基本软件包集的完整安装，且在安装过程中不需要访问 FTP 或 NFS 服务器的网络。要引导进入“基于 live CD 的文件系统”和系统救援模式，需要 `disc1` 和 `livefs`。`docs` 镜像包含所有支持语言的全部文档。大多数人会发现，如果你想在初始安装过程中安装一些软件包，`disc1`、`disc2` 和 `disc3` 就是你所需的全部；如果你倾向于在初始安装完成后再安装软件包，则只需 `disc1`。

FreeBSD 7.0-RELEASE 也可以从多家供应商购买 CD-ROM。其中一家将提供基于 FreeBSD 7.0 的产品的供应商是：
...
将任何现有系统升级到 FreeBSD 7.0-RELEASE 构成主版本升级，因此无论你使用哪种方法更新旧系统，都应重新安装机器上已安装的任何 Ports。这将避免在未来的 Ports 升级中重建某个 Port 但不重建链接到它的其他 Port 时，二进制文件链接到不一致的库集合。可以使用以下命令完成：

```sh
# portupgrade -faP
```

在更新系统之后。注意，用于协助此操作或下方 FreeBSD Update 说明的部分工具默认未安装（例如 portupgrade、gpg 或类似工具如 portmaster）。
