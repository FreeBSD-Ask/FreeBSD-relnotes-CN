...
对于所有支持的 SCSI 控制器，提供对 SCSI-I 和 SCSI-II 外设的全面支持，包括硬盘、光盘、磁带驱动器（包括 DAT 和 8mm Exabyte）、中介交换机、处理器目标设备和 CD-ROM 驱动器。支持读取的 WORM 设备（支持 CD-ROM 命令）可以通过 CD-ROM 驱动程序进行只读访问。WORM/CD-R/CD-RW 写入支持由 cdrecord 提供，且该工具可在 Ports 树中找到。
