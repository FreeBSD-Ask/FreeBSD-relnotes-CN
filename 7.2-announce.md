...
* 支持应用内存的完全透明使用 superpages
* 支持每个 Jail 使用多个 IPv4 和 IPv6 地址
* [csup(1)](http://www.freebsd.org/cgi/man.cgi?query=csup&sektion=1&manpath=FreeBSD+7.2-stable) 现在支持 CVSMode 来获取完整的 CVS 仓库
...
>**注意**
>
> 在测试周期后期发现，部分机器无法将 i386 disc1 识别为可引导介质（它们只是直接跳过，尝试从下一个引导设备引导）。所有受影响的机器都能识别其他光盘为可引导介质。如果你的机器存在此问题，从 bootonly 或 livefs 引导，然后在 sysinstall 启动后换入 disc1 应该可以工作。

FreeBSD 7.2-RELEASE 也可以从多家供应商购买 CD-ROM 或 DVD。其中一家将提供基于 FreeBSD 7.2 的产品的供应商是：
...
[freebsd-update(8)](http://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&manpath=FreeBSD+7.2-stable) 工具支持对运行早期 FreeBSD 发行版的 i386 和 amd64 系统进行二进制升级。运行 7.0-RELEASE、7.1-RELEASE、7.2-BETA、7.2-RC1 或 7.2-RC2 的系统可按以下方式升级：
...
早期 FreeBSD 发行版（FreeBSD 6.x）的用户也可以使用 `freebsd-update` 升级到 FreeBSD 7.2，但在第二次执行“freebsd-update install”之后将被提示重新构建所有第三方应用程序（例如从 Ports 树安装的任何内容），原因是 FreeBSD 6.x 和 FreeBSD 7.x 之间系统库的差异。
...
许多公司捐赠了设备、网络访问权限或人力时间来支持 FreeBSD 7.2 的发行工程活动，包括 The FreeBSD Foundation、Hewlett-Packard、Yahoo!、NetApp、Internet Systems Consortium 和 Sentex Communications。
