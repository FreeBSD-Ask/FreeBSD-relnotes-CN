...
[zfs(8)](http://www.freebsd.org/cgi/man.cgi?query=zfs&sektion=8&manpath=FreeBSD+8.4-RELEASE) `send` 命令现在在指定参数 `-v` 时报告传输的数据大小（以字节为单位）。[[r235952](http://svn.freebsd.org/viewvc/base?view=revision&revision=235952)
