...
在具备互联网邮件连接的机器上，提交漏洞报告的首选方法是使用 `send-pr` 命令，或访问 [http://www.FreeBSD.org/send-pr.html](http://www.FreeBSD.org/send-pr.html) 使用 CGI 脚本提交报告。 我们忠实的漏洞记录程序会尽职归档通过这种方式提交的漏洞报告，并且我们会尽力尽快回应所有报告的问题。以这种方式归档的漏洞报告也会显示在我们网站的支持部分，既可作为漏洞报告，也能为其他用户提供可能需要注意的问题的“指引”。
