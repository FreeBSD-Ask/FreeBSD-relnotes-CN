...
[hpt27xx(4)](https://man.freebsd.org/cgi/man.cgi?query=hpt27xx&sektion=4&format=html) 驱动支持以下SAS 控制器：
...
[hptiop(4)](https://man.freebsd.org/cgi/man.cgi?query=hptiop&sektion=4&format=html) 驱动支持以下SAS 和 SATA RAID 控制器：
...
[hptiop(4)](https://man.freebsd.org/cgi/man.cgi?query=hptiop&sektion=4&format=html) 驱动还支持以下SAS 和 SATA RAID 控制器 （已停产）：
...
[hptmv(4)](https://man.freebsd.org/cgi/man.cgi?query=hptmv&sektion=4&format=html) 驱动支持以下ATA RAID 控制器：

*	HighPoint's RocketRAID 182x series

[hptnr(4)](https://man.freebsd.org/cgi/man.cgi?query=hptnr&sektion=4&format=html) 驱动支持以下SATA 控制器：

*	HighPoint's DC7280 series
*	HighPoint's Rocket R750 series

[hptrr(4)](https://man.freebsd.org/cgi/man.cgi?query=hptrr&sektion=4&format=html) 驱动支持以下RAID 控制器：
...
[isci(4)](https://man.freebsd.org/cgi/man.cgi?query=isci&sektion=4&format=html) 驱动提供对Intel C600 `SAS` 控制器.
...
[pst(4)](https://man.freebsd.org/cgi/man.cgi?query=pst&sektion=4&format=html) 驱动支持the Promise Supertrak SX6000 ATA hardware RAID 控制器。
...
[twa(4)](https://man.freebsd.org/cgi/man.cgi?query=twa&sektion=4&format=html) 驱动支持以下SATA RAID 控制器：
...
[twe(4)](https://man.freebsd.org/cgi/man.cgi?query=twe&sektion=4&format=html) 驱动支持以下PATA/SATA RAID 控制器：
...
[tws(4)](https://man.freebsd.org/cgi/man.cgi?query=tws&sektion=4&format=html) 驱动支持以下SATA/SAS RAID 控制器：
...
[ae(4)](https://man.freebsd.org/cgi/man.cgi?query=ae&sektion=4&format=html) 驱动支持Attansic/Atheros L2 PCIe 快速以太网 控制器，并已知支持以下硬件：
...
当前版本的 `atse` 驱动支持the 以太网 MegaCore as described in version 11.1 of Altera's documentation when the device is configured with internal FIFOs.
...
[axe(4)](https://man.freebsd.org/cgi/man.cgi?query=axe&sektion=4&format=html) 驱动支持ASIX Electronics AX88172/AX88178/AX88772/AX88772A/AX88772B/AX88760 based USB 以太网 适配器，包括：
...
[cue(4)](https://man.freebsd.org/cgi/man.cgi?query=cue&sektion=4&format=html) 驱动支持CATC USB-EL1210A based USB 以太网 适配器，包括：
...
`cxgb` 驱动支持基于T3 and T3B chipset的10 千兆 and 1 千兆以太网 适配器：

*	Chelsio 10GBase-CX4
*	Chelsio 10GBase-LR
*	Chelsio 10GBase-SR

[cxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbe&sektion=4&format=html) 驱动支持基于T6 ASIC的100Gb and 25Gb 以太网 适配器：
...
[cxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbe&sektion=4&format=html) 驱动支持基于T5 ASIC的40Gb, 10Gb and 1Gb 以太网 适配器：
...
[cxgbe(4)](https://man.freebsd.org/cgi/man.cgi?query=cxgbe&sektion=4&format=html) 驱动支持基于T4 ASIC的10Gb and 1Gb 以太网 适配器：
...
`cxgbev` 驱动支持基于T6 ASIC的Virtual Functions on 100Gb and 25Gb 以太网 适配器：
...
`cxgbev` 驱动支持基于T5 ASIC的Virtual Functions on 40Gb, 10Gb and 1Gb 以太网 适配器：
...
`cxgbev` 驱动支持基于T4 ASIC的Virtual Functions on 10Gb and 1Gb 以太网 适配器：
...
[em(4)](https://man.freebsd.org/cgi/man.cgi?query=em&sektion=4&format=html) 驱动支持基于Intel 82540, 82541ER, 82541PI, 82542, 82543, 82544, 82545, 82546, 82546EB, 82546GB, 82547, 82571, 82572, 82573, 82574, 82575, 82576, and 82580 controller chips的千兆以太网 适配器：
...
`genet` 驱动支持the 以太网 控制器 portion of the Broadcom BCM2711 on the Raspberry Pi 4 Model B and related systems. It utilizes the BCM54213PE PHY。
...
`kue` 驱动支持Kawasaki LSI KL5KLUSB101B based USB 以太网 适配器，包括：
...
[mlx4en(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx4en&sektion=4&format=html) 驱动支持以下network 适配器：

*	Mellanox ConnectX-2 (ETH)
*	Mellanox ConnectX-3 (ETH)

`mlx4ib` 驱动支持以下network 适配器：

*	Mellanox ConnectX-2 (IB)
*	Mellanox ConnectX-3 (IB)

[mlx5en(4)](https://man.freebsd.org/cgi/man.cgi?query=mlx5en&sektion=4&format=html) 驱动支持200Gb, 100Gb, 50Gb, 40Gb, 25Gb and 10Gb ethernet 适配器。
...
`mlx5ib` 驱动支持100Gb, 50Gb, 40Gb, 25Gb and 10Gb network 适配器。 ConnectX-4 supports: 10/20/25/40/50/56/100Gb/s speeds. ConnectX-4 LX supports: 10/25/40/50Gb/s speeds (and reduced power consumption):
...
`mxge` 驱动支持基于Myricom LANai Z8E chips的10 千兆以太网 适配器：
...
[nfe(4)](https://man.freebsd.org/cgi/man.cgi?query=nfe&sektion=4&format=html) 驱动支持以下NVIDIA MCP onboard 适配器：
...
[nge(4)](https://man.freebsd.org/cgi/man.cgi?query=nge&sektion=4&format=html) 驱动支持National Semiconductor DP83820 and DP83821 based 千兆以太网 适配器，包括：
...
[oce(4)](https://man.freebsd.org/cgi/man.cgi?query=oce&sektion=4&format=html) 驱动支持以下network 适配器：
...
`rtwn_usb` 驱动支持Realtek RTL8188CUS/RTL8188RU/RTL8188EU/RTL8192CU/RTL8192EU/RTL8812AU/RTL8821AU based USB wireless network 适配器，包括：
...
`rue` 驱动支持RealTek RTL8150 based USB 以太网 适配器，包括：
...
[sfxge(4)](https://man.freebsd.org/cgi/man.cgi?query=sfxge&sektion=4&format=html) 驱动支持all 10Gb 以太网 适配器 基于 Solarflare SFC9000 family 控制器。
...
[sis(4)](https://man.freebsd.org/cgi/man.cgi?query=sis&sektion=4&format=html) 驱动支持基于National Semiconductor DP83815 (MacPhyter) and DP83816 chips. Supported adapters include的Silicon Integrated Systems SiS 900 and SiS 7016 based 快速以太网 适配器 and embedded 控制器, as well as 快速以太网 适配器：
...
[ste(4)](https://man.freebsd.org/cgi/man.cgi?query=ste&sektion=4&format=html) 驱动支持Sundance Technologies ST201 based 快速以太网 适配器 and embedded 控制器，包括：
...
[bwi(4)](https://man.freebsd.org/cgi/man.cgi?query=bwi&sektion=4&format=html) 驱动支持Broadcom BCM43xx based wireless devices，包括：
...
[ral(4)](https://man.freebsd.org/cgi/man.cgi?query=ral&sektion=4&format=html) 驱动支持PCI/PCIe/CardBus wireless 适配器 基于 Ralink Technology chipsets，包括：
...
`rum` 驱动支持基于Ralink RT2501USB and RT2601USB chipsets, including的USB 2.0 wireless 适配器：
...
[run(4)](https://man.freebsd.org/cgi/man.cgi?query=run&sektion=4&format=html) 驱动支持以下wireless 适配器：
...
[ural(4)](https://man.freebsd.org/cgi/man.cgi?query=ural&sektion=4&format=html) 驱动支持基于Ralink Technology RT2500USB chipset, including的USB 2.0 wireless 适配器：
...
`ce` 驱动支持以下models of Tau-PCI/32 WAN 适配器：
...
`cp` 驱动支持以下models of Tau-PCI WAN 适配器：
...
`snd_fm801` 驱动支持基于following chipset的audio devices：
...
`snd_hda` 驱动支持基于following chipset的控制器 having PCI class 4 (multimedia) and subclass 3 (HDA), compatible with Intel HDA specification.

`snd_hda` 驱动支持more than two hundred different 控制器 and CODECs. There is no sense to list all of them here, as in most cases specific CODEC configuration and wiring are more important then type of the CODEC itself。
...
[uhci(4)](https://man.freebsd.org/cgi/man.cgi?query=uhci&sektion=4&format=html) 驱动支持all UHCI v1.1 compliant 控制器，包括：
...
[ubser(4)](https://man.freebsd.org/cgi/man.cgi?query=ubser&sektion=4&format=html) 驱动提供对the BWCT console management serial adapters.
...
`ng_ubt` 驱动支持all Bluetooth USB devices that conform with the Bluetooth specification v1.1，包括：
...
[ccr(4)](https://man.freebsd.org/cgi/man.cgi?query=ccr&sektion=4&format=html) 驱动支持基于T6 ASIC的the crypto accelerator engine included on 适配器：
