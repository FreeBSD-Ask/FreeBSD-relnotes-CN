...
强烈建议所有用户在安装 FreeBSD 之前查阅发布的勘误文档。勘误文档会更新"最新"信息，这些信息是在发布周期后期或发布之后发现的。通常，它包含已知的 bug、安全公告以及文档更正。FreeBSD 11.0-RELEASE 的最新勘误文档可以在 FreeBSD 官方网站上找到。
...
OpenCrypto 框架已更新，增加了 `AES-ICM` 和 `AES-GCM` 模式，这些模式也添加到了 [aesni(4)](http://www.freebsd.org/cgi/man.cgi?query=aesni&sektion=4&manpath=freebsd-release-ports) 驱动程序中。[(r275732)](http://svn.freebsd.org/viewvc/base?view=revision&revision=275732)（由 FreeBSD 基金会和 Netgate 提供赞助）
...
已引入对 PCI 单根 I/O 虚拟化（SR-IOV）的支持，允许为支持 SR-IOV 的设备驱动程序创建 PCI 虚拟功能（VF）。创建和配置 VF 的方法，请参见 [iovctl(8)](http://www.freebsd.org/cgi/man.cgi?query=iovctl&sektion=8&manpath=freebsd-release-ports)。[(r279463)](http://svn.freebsd.org/viewvc/base?view=revision&revision=279463)（由 Sandvine, Inc. 提供赞助）
