...
有关通过 [freebsd-update(8)](http://www.freebsd.org/cgi/man.cgi?query=freebsd-update&sektion=8&manpath=freebsd-release-ports) 升级的信息，请参阅安装页面中的“二进制升级部分”。
...
无线网络堆栈已修改，默认不再显示物理无线设备。要查看系统上可用的无线设备，请运行 `sysctl net.wlan.devices`。 [(r287197)](http://svn.freebsd.org/viewvc/base?view=revision&revision=287197) (由Netflix，Nginx, Inc. 赞助)
...
添加了新的内核配置选项 `EM_MULTIQUEUE`，它启用 [em(4)](http://www.freebsd.org/cgi/man.cgi?query=em&sektion=4&manpath=freebsd-release-ports) 驱动程序的多队列支持。[(r283959)](http://svn.freebsd.org/viewvc/base?view=revision&revision=283959)（由 Limelight Networks 赞助）
...
[ahci(4)](http://www.freebsd.org/cgi/man.cgi?query=ahci&sektion=4&manpath=freebsd-release-ports) 驱动程序已更新，增加了对支持 NCQ TRIM 的驱动器的支持。[(r298002)](http://svn.freebsd.org/viewvc/base?view=revision&revision=298002)（由Netflix赞助）
...
[cam(4)](http://www.freebsd.org/cgi/man.cgi?query=cam&sektion=4&manpath=freebsd-release-ports) 驱动程序已更新，允许根据工作负载和驱动器特性调优 I/O 调度。此选项默认为关闭，可以通过将 `option CAM_IOSCHED_ADAPTIVE` 选项添加到内核配置并重新编译内核来启用。[(r298002)](http://svn.freebsd.org/viewvc/base?view=revision&revision=298002)（由Netflix赞助）
...
已添加对原子数据报的 IP 标识支持（RFC 6864）。可以通过 [sysctl(8)](http://www.freebsd.org/cgi/man.cgi?query=sysctl&sektion=8&manpath=freebsd-release-ports) `net.inet.ip.rfc6864` 启用或禁用此功能，默认启用。[(r280971)](http://svn.freebsd.org/viewvc/base?view=revision&revision=280971)（由Netflix，Nginx，Inc.赞助）
