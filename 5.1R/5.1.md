...
[tcpdump(1)](http://www.freebsd.org/cgi/man.cgi?query=tcpdump&sektion=1&manpath=FreeBSD+5.1-release) 已更新至版本 3.7.2。[已合并] 它现在还支持 `-L` 参数来列出接口上可用的数据链路类型，以及选项 `-y` 来指定捕获数据包时使用的数据链路类型。
...
[pkg_create(1)](http://www.freebsd.org/cgi/man.cgi?query=pkg_create&sektion=1&manpath=FreeBSD+5.1-release) 现在支持选项 `-C`，该选项允许软件包注册与其冲突的其他软件包列表。如果已安装其中一个冲突的软件包，则 [pkg_add(1)](http://www.freebsd.org/cgi/man.cgi?query=pkg_add&sektion=1&manpath=FreeBSD+5.1-release) 将拒绝安装。`-f` 参数可以用来覆盖此冲突检查。
