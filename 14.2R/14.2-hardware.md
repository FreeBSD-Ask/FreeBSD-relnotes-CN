...
[sym(4)](https://man.freebsd.org/cgi/man.cgi?query=sym&sektion=4&format=html)
 驱动支持以下 Symbios/LSI Logic PCI SCSI
控制器：
...
Altera 文档中所述，当设备配置为使用内部 FIFO 时。
...
NetXtreme-E 千兆以太网控制器芯片系列的各种 NIC 提供支持，包括
以下：
...
[bxe(4)](https://man.freebsd.org/cgi/man.cgi?query=bxe&sektion=4&format=html)
 驱动为基于 QLogic NetXtreme II
10Gb 以太网控制器芯片系列的各种 NIC 提供支持，包括以下：
...
[genet(4)](https://man.freebsd.org/cgi/man.cgi?query=genet&sektion=4&format=html)
 驱动支持 Broadcom BCM2711 的以太网控制器部分，用于树莓派 4 Model B 及相关系统。
它采用 BCM54213PE PHY。
...
此系列中带 SFP28/QSFP28 接口的大多数适配器，其固件要求使用 Intel 认证模块；这些认证模块如下所列。
此限定检查无法由驱动禁用。
...
此系列中带 SFP+/SFP28/QSFP+ 接口的大多数适配器，其固件要求使用 Intel 认证模块；这些认证模块如下所列。
此限定检查无法由驱动禁用。
...
[msk(4)](https://man.freebsd.org/cgi/man.cgi?query=msk&sektion=4&format=html)
 驱动为基于 Marvell/SysKonnect Yukon II 千兆以太网控制器芯片的各种 NIC 提供支持，包括：
...
基于 RTL810xE 和 RTL8111 的快速以太网和千兆以太网适配器，包括：
...
快速以太网适配器和嵌入式控制器，以及基于 National Semiconductor DP83815 (MacPhyter) 和 DP83816
芯片的快速以太网适配器。
支持的适配器包括：
...
[stge(4)](https://man.freebsd.org/cgi/man.cgi?query=stge&sektion=4&format=html)
 驱动为基于 Sundance/Tamarack TC9021 千兆以太网控制器芯片的
各种 NIC 提供支持，包括：
...
[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html)
 驱动支持基于 Alteon Tigon I 和 II 芯片的适配器。
[ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4&format=html)
 驱动已在以下适配器上测试：
...
该设备具有一个称为“Zero-CD”的模式，其中包含 Microsoft Windows 的驱动程序；这是设备的默认模式。
[uhso(4)](https://man.freebsd.org/cgi/man.cgi?query=uhso&sektion=4&format=html)
 驱动自动将设备从“Zero-CD”模式切换为调制解调器模式。此行为可通过 **sysctl(8)** 将
`hw.usb.uhso.auto_switch` 设置为 0 来禁用。

[urndis(4)](https://man.freebsd.org/cgi/man.cgi?query=urndis&sektion=4&format=html)
 驱动支持许多 Android 设备的 RNDIS“tethering”功能。
...
使用较旧 Broadcom 芯片组（BCM4301、BCM4303 和 BCM4306 rev 2）的用户必须使用 **bwi(4)**，因为 v4 版本的固件不支持这些芯片。
较新的固件过大，无法放入这些旧芯片。

[ipw(4)](https://man.freebsd.org/cgi/man.cgi?query=ipw&sektion=4&format=html)
 驱动支持 Intel PRO/Wireless 2100a/b MiniPCI 网络适配器。
...
[rsu(4)](https://man.freebsd.org/cgi/man.cgi?query=rsu&sektion=4&format=html)
 驱动为 Realtek RTL8188SU/RTL8192SU USB IEEE 802.11b/g/n 无线网络适配器提供支持，
包括：
...
[rum(4)](https://man.freebsd.org/cgi/man.cgi?query=rum&sektion=4&format=html)
 驱动支持基于 Ralink RT2501USB 和 RT2601USB 芯片组的 USB 2.0 无线
适配器，
包括：
...
[upgt(4)](https://man.freebsd.org/cgi/man.cgi?query=upgt&sektion=4&format=html)
 驱动支持 USB 2.0 Conexant/Intersil PrismGT 系列无线适配器（基于 GW3887 芯片组），包括：
...
[ural(4)](https://man.freebsd.org/cgi/man.cgi?query=ural&sektion=4&format=html)
 驱动支持基于 Ralink Technology
RT2500USB 芯片组的 USB 2.0 无线适配器，包括：
...
此接口作为硬件的抽象，允许使用不同的 UART 进行串行通信。
...
[ucycom(4)](https://man.freebsd.org/cgi/man.cgi?query=ucycom&sektion=4&format=html)
 驱动目前支持以下包含 Cypress USB 转 RS232 桥接芯片的设备：
...
部分板载 CS4610 芯片附带 CS423x ISA 编解码器，而非 CS4297 AC97 编解码器。
此类配置尚未被
[snd_csa(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_csa&sektion=4&format=html)
 驱动支持。
...
[snd_hda(4)](https://man.freebsd.org/cgi/man.cgi?query=snd_hda&sektion=4&format=html)
 驱动支持具有 PCI 类 4（多媒体）和子类 3（HDA）的控制器，兼容 Intel HDA 规范。
...
[ulpt(4)](https://man.freebsd.org/cgi/man.cgi?query=ulpt&sektion=4&format=html)
 驱动为 USB 打印机和并行打印机转换电缆提供支持，包括以下：
...
通过 USB、I2C、Hyper-V 或 Apple SPI
...
* PS/2 鼠标及兼容设备，
包括许多笔记本电脑指点设备（[psm(4)](https://man.freebsd.org/cgi/man.cgi?query=psm&sektion=4&format=html) 驱动）
