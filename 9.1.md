...
所有用户在安装 FreeBSD 之前，建议先查阅发布勘误。勘误文档会随着发布周期结束或发布后发现的“最新”信息而更新。通常，它包含已知的 bug、安全公告和文档修正。FreeBSD 9.1-RELEASE 的最新勘误可以在 FreeBSD 网站上找到。
...
[sparc64] `PREEMPTION` 内核选项现在在 `GENERIC` 内核中启用。[r235992](http://svn.freebsd.org/viewvc/base?view=revision&revision=235992)
...
[amd64] 已实现对 AMD 处理器系列 10h 和 12h 的 Erratum 721 变通方案。在高度特定和详细的内部时序条件集下，处理器可能会在一长串 push 和/或近调用指令，或一长串 pop 和/或近返回指令后错误地更新栈指针。[r233798](http://svn.freebsd.org/viewvc/base?view=revision&revision=233798)
...
简单的脚本 `zfsboottest.sh` 已添加到源代码树中。该脚本用于检查在使用 ZFS 作为根文件系统时系统是否配置正确。脚本位于 **/usr/src/tools/tools/zfsboottest.sh**，并使用同目录下的二进制程序 `zfsboottest`。[r227705](http://svn.freebsd.org/viewvc/base?view=revision&revision=227705)
...
函数 [fdlopen(3)](https://man.freebsd.org/cgi/man.cgi?query=fdlopen&sektion=3&format=html) 已实现。这是个用于 [rtld(1)](https://man.freebsd.org/cgi/man.cgi?query=rtld&sektion=1&format=html) 通过文件涉及符加载共享对象的接口。[r230410](http://svn.freebsd.org/viewvc/base?view=revision&revision=230410)
